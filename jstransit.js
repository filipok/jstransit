...
function readTimerFile(e) {
	...
	reader.onload = function (e) {
		...
		xhttp.onreadystatechange = function() {
		    ...
		    for(var p = 0; p < way_refs.length; p++){
		    	var selector = '[id="' + way_refs[p] + '"]';
		    ...
		    for(var p = 0; p < waypoint_refs.length; p++){
		    	var selector = '[id="' + waypoint_refs[p] + '"]';
		    	var platform = xmlDoc.querySelectorAll(selector)[0];
		    	var lat = parseFloat(platform.getAttribute("lat"));
		    	point_lats.push(lat);
		    	var lon = parseFloat(platform.getAttribute("lon"));
		    ...
		    // TODO aici pot face un map.setView() după un getBounds() sau
		    // din coords cu max/min
		    ...
		    var all_nodes = xmlDoc.getElementsByTagName("node");
		    for(var p = 0; p < plat_refs.length; p++){
		    	var selector = '[id="' + plat_refs[p].toString() + '"]';
		    	var platform = xmlDoc.querySelectorAll(selector)[0];
		    	var lat = parseFloat(platform.getAttribute("lat"));
		    	lats.push(lat);
		    	var lon = parseFloat(platform.getAttribute("lon"));
